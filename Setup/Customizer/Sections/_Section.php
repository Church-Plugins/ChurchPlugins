...
use ChurchPlugins\Setup\Customizer\Controls\Heading;
use ChurchPlugins\Setup\Customizer\Controls\Number;
use ChurchPlugins\Setup\Customizer\Controls\Radio;
use ChurchPlugins\Setup\Customizer\Controls\RangeSlider;
use ChurchPlugins\Setup\Customizer\Controls\Separator;
use ChurchPlugins\Setup\Customizer\Controls\Toggle;

/**
 * Church Plugins Customizer Section Abstract.
 * Extend this when you are trying to create a new Church Plugins Section
...
abstract class _Section {
	...
	final public function __construct() {
		...
		add_action( "cp_customizer_register_{$this->ID}_settings", [ $this, 'register_settings' ], 10, 2 );
		add_filter( 'cp_customizer_pre_sections', [ $this, 'register' ], 10, 2 );

		// Append this section CSS template
		add_filter( 'cp_customizer_css_template', [ $this, 'setup_css_template' ], $this->queue_priority );
		add_filter( "cp_customizer_section_{$this->ID}_defaults", [ $this, 'get_defaults' ], 10 );

		// Create the Ghost Options
		add_filter( 'cp_customizer_pre_get_option', [ $this, 'filter_settings' ], 10, 2 );

		// By Default Invoking a new Section will load, unless `load` is set to false
		if ( true === (bool) $this->load ) {
			churchplugins()->setup->customizer->load_section( $this );
		}
	}
	...
	public function register_settings( WP_Customize_Section $section, WP_Customize_Manager $manager ) {
		$customizer = churchplugins()->setup->customizer;
	...
	public function should_include_setting_css( $setting, $section_id = null ) {
		...
		$setting_value = churchplugins()->setup->customizer->get_option( [ $section_id, $setting ] );
		$section       = churchplugins()->setup->customizer->get_section( $section_id );
	...
	public function get_option( $setting ) {
		...
		return churchplugins()->setup->customizer->get_option( [ $this->ID, $setting ] );
	}
	...
	/**
	 * Hooks to the `cp_customizer_pre_get_option`. This applies the `$this->create_ghost_settings()` method
	 * to the settings on the correct section.
	 *
	 * @param  array $settings  Values from the Database from Customizer actions.
	 * @param  array $search	Indexed search @see \ChurchPlugins\Setup\Customizer\_Init::search_var().
	 *
	 * @return array
	 */
	...
	public function filter_arguments( $arguments ) {
		 ...
		 */
		$arguments = apply_filters( 'cp_customizer_section_arguments', $arguments, $this );
		 ...
		 * @param static				 $instance  The section instance we are dealing with.
		 */
		return apply_filters( "cp_customizer_section_{$section_slug}_arguments", $arguments, $this );
	}
	...
	public function filter_defaults( $settings ) {
		 ...
		 * @param _Section $section The section object.
		 */
		$settings = apply_filters( 'cp_customizer_section_default_settings', $settings, $this );
		 ...
		 * @param _Section $section The section object.
		 */
		$settings = apply_filters( "cp_customizer_section_{$section_slug}_default_settings", $settings, $this );
	...
	/**
	 * Sugar function that returns the results of \ChurchPlugins\Setup\Customizer\_Init->get_section_url() for the current section.
	 *
	 * @since 1.0.6
	 *
	...
	public function get_section_url() {
		return churchplugins()->setup->customizer->get_section_url( $this->ID );
	}

	/**
	 * Sugar function that returns the results of \ChurchPlugins\Setup\Customizer\_Init->get_section_link() for the current section.
	...
	public function get_section_link( $link_text ) {
		return churchplugins()->setup->customizer->get_section_link( $this->ID, $link_text );
	}

	/**
	 * Sugar function that returns the results of \ChurchPlugins\Setup\Customizer\_Init->get_settings_url()
	...
	public function get_setting_url( $setting ) {
		return churchplugins()->setup->customizer->get_setting_url( $this->ID, $setting );
	}
	...
	public function get_setting_link( $setting, $link_text ) {
		return churchplugins()->setup->customizer->get_setting_link( $this->ID, $setting, $link_text );
	}
	...
	public function filter_content_settings( $arguments ) {
		 ...
		 * @param static				 $instance  The section instance we are dealing with.
		 */
		$arguments = apply_filters( 'cp_customizer_section_content_settings', $arguments, $this );
		 ...
		 * @param static				 $instance  The section instance we are dealing with.
		 */
		$arguments = apply_filters( "cp_customizer_section_{$section_slug}_content_settings", $arguments, $this );
	...
	public function filter_content_headings( $arguments ) {
		 ...
		 * @param static				 $instance  The section instance we are dealing with.
		 */
		$arguments = apply_filters( 'cp_customizer_section_content_headings', $arguments, $this );
		 ...
		 * @param static				 $instance  The section instance we are dealing with.
		 */
		return apply_filters( "cp_customizer_section_{$section_slug}_content_headings", $arguments, $this );
	}
	...
	public function get_accepted_control_types() {
		 ...
		 */
		return apply_filters( 'cp_customizer_accepted_control_types', $accepted_control_types, $this );
	}
	...
	public function filter_content_controls( $arguments ) {
		 ...
		 * @param static				 $instance  The section instance we are dealing with.
		 */
		$arguments = apply_filters( 'cp_customizer_section_content_controls', $arguments, $this );
		 ...
		 * @param static				 $instance  The section instance we are dealing with.
		 */
		return apply_filters( "cp_customizer_section_{$section_slug}_content_controls", $arguments, $this );
	}
	...
	public function filter_css_template( $template ) {
		 ...
		 * @param static				 $instance  The section instance we are dealing with.
		 */
		$template = apply_filters( 'cp_customizer_section_css_template', $template, $this );
		 ...
		 * @param static				 $instance  The section instance we are dealing with.
		 */
		$template = apply_filters( "cp_customizer_section_{$section_slug}_css_template", $template, $this );
	...
	public function get_rgb_color( $option, $section = null ) {
		$color = is_null( $section )
			? churchplugins()->setup->customizer->get_option( [ $this->ID, $option ] )
			: churchplugins()->setup->customizer->get_option( [ $section, $option ] );

		$color_obj = new \ChurchPlugins\Utils\Color( $color );

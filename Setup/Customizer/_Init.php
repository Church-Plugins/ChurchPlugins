<?php
namespace ChurchPlugins\Setup\Customizer;
...
/**
 * Church Plugins Customizer class
 *
...
final class _Init {
	...
	public function __construct() {
		 ...
		 */
		$print_styles_action = apply_filters( 'cp_customizer_print_styles_action', 'wp_enqueue_scripts' );

		add_action( $print_styles_action, [ $this, 'inline_style' ], 15 );

//		add_filter( "default_option_{$this->ID}", [ $this, 'maybe_fallback_get_option' ] );

		add_action( 'cp_core_loaded', [ $this, 'load_sections' ] );
	}
	...
	public function load_sections() {
		...
		$sections = apply_filters( 'cp_customizer_sections', $sections );
	...
	public function maybe_fallback_get_option( $sections ) {
		...
		return get_option( 'cp_customizer', [] );
	}

	/**
	 * Loads a Section to the Customizer on Church Plugin's Panel
	 *
	 * @since  1.0.6
	 *
	 * @param  object $section An Object that extends the Abstract `_Init__Section`
	 *
	 * @return bool
	 */
	public function load_section( $section ) {
		// You can only add a section if it extends the abstract Section
		if ( ! is_object( $section ) || ! in_array( 'ChurchPlugins\Setup\Customizer\Sections\_Section', class_parents( $section ) ) ) {
			return false;
		}
	...
	public function get_loaded_sections() {
		 ...
		 * @param self  $customizer
		 */
		$this->sections_class = apply_filters( 'cp_customizer_sections_class', $this->sections_class, $this );
	 ...
	 *
	 * @return boolean|_Init__Section The requested section or boolean false if not found.
	 */
	...
	public function get_option( $search = null, $default = null ) {
		...
		foreach ( $this->get_loaded_sections() as $section ) {
			 ...
			 */
			$defaults[ $section->ID ] = apply_filters( "cp_customizer_section_{$section->ID}_defaults", [] );
			...
			$defaults[ $section->ID ] = apply_filters( "cp_customizer_section_{$section->ID}_defaults", $settings );
		 ...
		 * @param array $search
		 */
		$sections = apply_filters( 'cp_customizer_pre_get_option', $sections, $search );
		 ...
		 * @param array $sections
		 */
		$option = apply_filters( 'cp_customizer_get_option', $option, $search, $sections );
	...
	public function customize_controls_print_footer_scripts() {
		 ...
		 *
		 * @param _Init $customizer The current instance of _Init.
		 */
		do_action( 'cp_enqueue_customizer_scripts', $this );
	}

	/**
	 * Print the CSS for the customizer on `wp_print_footer_scripts`
	 *
	 * @since 1.0.6 Moved the template building code to the `get_styles_scripts` method.
	 *
	 * @return void
	 */
	...
	public function shortcode_inline_style() {
		 ...
		 */
		$should_print = apply_filters( 'cp_customizer_should_print_shortcode_customizer_styles', false );
	...
	public function widget_inline_style() {
		 ...
		 * @param boolean $should_print Whether the inline styles should be printed on screen.
		 */
		$should_print = apply_filters( 'cp_customizer_should_print_widget_customizer_styles', false );
	 ...
	 *
	 * @since 1.0.6 Added the `$force` parameter to force the print of the style inline.
	 *
	...
	public function inline_style( $force = false ) {
		 ...
		 */
		$css_template = trim( apply_filters( 'cp_customizer_css_template', '' ) );
		 ...
		 */
		$sheets = apply_filters( 'cp_customizer_inline_stylesheets', $sheets, $css_template );
		...
		foreach ( array_reverse( $sheets ) as $sheet ) {
			if ( wp_style_is( $sheet ) ) {
				 ...
				 */
				do_action( 'cp_customizer_before_inline_style', $sheet, $inline_style );
	...
	public function register( WP_Customize_Manager $customizer ) {
		 ...
		 * @param _Init  $customizer
		 */
		$this->panel = apply_filters( 'cp_customizer_panel', $this->register_panel(), $this );
		 ...
		 * @param _Init $customizer
		 */
		$this->sections = apply_filters( 'cp_customizer_pre_sections', $this->sections, $this );
		...
		foreach ( $this->sections as $id => $section ) {
			 ...
			 * @param _Init    $customizer The current customizer instance.
			 */
			do_action( "cp_customizer_register_{$id}_settings", $this->sections[ $id ], $this->manager, $this );
		}
		 ...
		 * @param _Init $customizer
		 */
		$this->sections = apply_filters( 'cp_customizer_sections', $this->sections, $this );
	...
	/**
	 * Register the base Panel for Church Plugins Sections to be attached to
	 *
	 * @since 1.0.6
	 *
	...
	private function register_panel() {
		...
		$panel_args = [
			'title'       => esc_html__( 'Church Plugins', 'church-plugins' ),
			'description' => esc_html__( 'Use the following panel of your customizer to change the styling of your Church Plugins pages.', 'church-plugins' ),
		 ...
		 * @param _Init $customizer
		 */
		$panel_args = apply_filters( 'cp_customizer_panel_args', $panel_args, $this->ID, $this );
	...
	public function get_panel_url() {
		$query['autofocus[panel]'] = 'cp_customizer';
		return add_query_arg( $query, admin_url( 'customize.php' ) );
	}

	/**
	 * Returns an HTML link directly to the (opened) CP Customizer panel
	 *
	 * @since 1.0.6
	 *
	 * @param string $link_text The (pre)translated text for the link.
	 *
	 * @return string The HTML anchor element, linking to the CP Customizer panel.
	...
	public function register_section( $id, $args ) {
		 ...
		 * @param _Init $customizer
		 */
		$section_id = apply_filters( 'cp_customizer_section_id', $id, $this );
		 ...
		 * @param _Init $customizer
		 */
		$section_args = apply_filters( 'cp_customizer_section_args', $args, $section_id, $this );
	...
	/**
	 * Returns a URL to the a specific CP Customizer section.
	 *
	 * @since 1.0.6
	 *
	 * @param string $section The slug for the desired section.
	 *
	 * @return string The URL to the CP Customizer section.
	 */
	...
	/**
	 * Gets the HTML link to a section in the CP Customizer.
	 *
	 * @since 1.0.6
	 *
	 ...
	 *
	 * @return string The HTML anchor element, linking to the CP Customizer section.
	 *                An empty string is returned if missing a parameter.
	 */
	...
	/**
	 * Gets the link to the a specific control/setting in the CP Customizer.
	 *
	 * @since 1.0.6
	 *
	 * @param string $section   The section "slug" to link into.
	 ...
	 *
	 * @return string The HTML anchor element, linking to the CP Customizer setting.
